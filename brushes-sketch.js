(function() {
    ...
    function disegnaSuTela(dimensione, trasparenza, custom, velocita, volume) {
        ...
        // FIX: Ripristinata la logica di disegno per tutti i pennelli
        switch (tipoPennello) {
            case 1: // Bubbles
                let freq = map(easedCustom,0,1,1,15); 
            ...
            case 3: // Organic
                telaDisegno.push(); 
                telaDisegno.translate(mouseX,mouseY); 
                let sens=map(easedCustom,0,1,50,2500); 
                telaDisegno.beginShape(); 
                for (let a=0; a<TWO_PI; a+=0.3) {
                    telaDisegno.vertex(cos(a)*(dimensione/2 + noise(a*5, millis()*0.005) * (volume*sens)), sin(a)*(dimensione/2 + noise(a*5, millis()*0.005) * (volume*sens)));
                }
            ...
            case 4: // Orbiting Dots
                telaDisegno.push(); 
                telaDisegno.translate(mouseX,mouseY); 
                let velRotazione=map(easedCustom,0,1,0.01,0.5); 
                for (let i=0; i<3; i++) {
                    telaDisegno.ellipse(cos(TWO_PI*(i/3)+angolo)*dimensione/1.5, sin(TWO_PI*(i/3)+angolo)*dimensione/1.5, dimensione/5);
                }
                angolo += velRotazione; 
                telaDisegno.pop(); 
                break;
            case 5: // Spray Paint
                let densita=map(easedCustom,0,1,5,250); 
                for (let i=0; i<densita; i++) { 
                    let r = sqrt(random(1)) * (dimensione / 2); 
                    let a = random(TWO_PI); 
            ...
            case 6: // Swellings
                ...
                telaDisegno.noFill();
                // Rimosso il controllo sulla velocità per renderlo sempre attivo
            ...
            case 8: // Crystallize
                ...
                // Rimosso il controllo sulla velocità
    ...
    window.keyPressed = function() {
        ...
        if (key >= '0' && key <= '9') {
            ...
            etichettaPennello.html(nomiPennelli[tipoPennello]);
            // Resetta lo stato del pennello Web se non è selezionato
            if (tipoPennello !== 7) {
                puntiPrecedenti = [];
            }
        }
