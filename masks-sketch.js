(function() {
    ...
    class Scene {
        ...
        updateVolume() {
            ...
            // 1. Ottiene il volume dal microfono
            let micVolume = mic.getLevel() * sensitivity;

            // 2. Ottiene un "volume" fittizio dalla posizione del mouse
            let mouseVolume = 0;
            if (mouseX > 0 && mouseX < width && mouseY > 0 && mouseY < height) {
                mouseVolume = map(mouseX, 0, width, 0, 1.0);
            }
            
            // 3. Usa il valore più alto tra i due come input finale
            let finalInput = max(micVolume, mouseVolume);

            // 4. Ammorbidisce il valore per un'animazione fluida
    ...
    class RobotScene extends Scene {
        draw() {
            ...
            // Nota: la bocca del robot reagisce solo al vero audio (FFT)
    ...
    class JesterScene extends Scene {
        draw() {
            ...
            fill(0);
            rectMode(CORNER);
            rect(-90, 0, 20, tearLength);
            rect(70, 0, 20, tearLength);
