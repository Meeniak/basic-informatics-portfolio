// Funzione "contenitore" per garantire che p5.js si avvii correttamente
const sketch = (p) => {
    ...
    p.draw = function() {
        ...
        if (typeof xyPoint === 'undefined' || xyPoint.length === 0) {
            p.fill(255, 0, 0); p.textAlign(p.CENTER, p.CENTER); p.textSize(16);
    ...
    // ========= STILI CREATIVI (TUA LOGICA) =========
    function gestisciTraccia(show, sCol, sWeight, alpha) { if (show) { let c=p.color(sCol); c.setAlpha(alpha); p.stroke(c); p.strokeWeight(sWeight); } else { p.noStroke(); }}
    function stileOrbitePulsanti(w,h,speed,c1,c2,sCol,alpha,showStroke,sWeight){ gestisciTraccia(showStroke,sCol,sWeight,alpha); for(let i=0;i<3;i++){ let col=(i%2===0)?c1:c2; col.setAlpha(alpha); p.fill(col); let pulse=p.sin(p.frameCount*speed+i*120); let size=p.map(pulse,-1,1,5,h); let angle=p.frameCount*speed*0.5+i*120; let radius=w/2*p.sin(p.frameCount*speed*0.2+i*120); p.ellipse(p.cos(angle)*radius,p.sin(angle)*radius,size,size);}}
    function stileAghiRotanti(w,numLines,speed,c1,c2,alpha){ p.noFill(); p.strokeWeight(3); for(let i=0;i<numLines;i++){ let col=(i%2===0)?c1:c2; col.setAlpha(alpha); p.stroke(col); p.push(); p.rotate(i*(360/numLines)+p.frameCount*speed); p.line(0,0,w,0); p.pop();}}
    function stileCristalliCoordinati(w,crystalCount,speed,c1,c2,sCol,alpha,showStroke,sWeight){ gestisciTraccia(showStroke,sCol,sWeight,alpha); let time=p.frameCount*speed; p.rotate(time*0.5); let sizePulse=p.sin(time*0.8); let raggioBase=p.map(sizePulse,-1,1,w*0.1,w*0.4); for(let i=0;i<crystalCount;i++){ p.push(); let raggio=raggioBase; if(i===0){raggio*=1.5; c1.setAlpha(alpha); p.fill(c1);} else{ let orbitRadius=w*0.4; let orbitAngle=time*-1+(i*360/(crystalCount-1)); p.translate(orbitRadius*p.cos(orbitAngle),orbitRadius*p.sin(orbitAngle)); p.rotate(time*1.5); c2.setAlpha(alpha*0.9); p.fill(c2);} disegnaTriangoloEquilatero(0,0,raggio); p.pop();}}
    ...
    function updateControlsVisibility() {
        p.select('#group-h').hide(); p.select('#group-s2-complex').hide();
        p.select('#group-s3-complex').hide(); p.select('#group-s4-complex').hide();
        p.select('#group-s5-complex').hide(); p.select('#group-stroke-options').hide();
        switch (currentStyle) {
...
// Avvia lo sketch solo quando il DOM è completamente caricato.
document.addEventListener('DOMContentLoaded', () => {
    // Verifica che l'array dei punti esista prima di avviare lo sketch
    if (typeof xyPoint !== 'undefined') {
        new p5(sketch);
    } else {
        console.error("L'array 'xyPoint' non è stato trovato. Assicurati che il file 'lettere.js' sia caricato correttamente prima di 'sketch.js'.");
        const canvasWrapper = document.getElementById('canvas-wrapper');
        if(canvasWrapper) {
            canvasWrapper.innerHTML = "<p style='color:red; padding:20px;'>Errore: Dati della lettera non trovati.</p>";
        }
    }
});
