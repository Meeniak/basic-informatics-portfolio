// Funzione auto-eseguibile per incapsulare lo script in modo sicuro
(function() {
    // --- VARIABILI GLOBALI ---
    // Controlli UI
    ...
    window.setup = function() {
        ...
        canvas.parent(canvasWrapper); // Associa il canvas al suo contenitore
        
        angleMode(DEGREES);

        // --- INIZIALIZZAZIONE CONTROLLI UI ---
        // Ogni elemento viene creato e subito associato al suo div contenitore
        ...
        // Applica lo stile a tutti gli slider per coerenza
        selectAll('input[type="range"]').forEach(s => s.style('width', '100%'));

        // Inizializza l'array per le code delle comete
    ...
    window.draw = function() {
        background(255); // Sfondo bianco di default
        translate(width / 2, height / 2); // Centra il disegno
    ...
    function stileCometaScintillante(w, tailLength, speed, c1, c2, alpha, pointIndex) {
        ...
        while(tail.length > tailLength){ tail.pop(); } 
        noStroke(); 
    ...
    function disegnaTriangoloEquilatero(x, y, raggio) {
        ...
        let x1=x+raggio*cos(a1); let y1=y+raggio*sin(a1); 
        let x2=x+raggio*cos(a2); let y2=y+raggio*sin(a2); 
        let x3=x+raggio*cos(a3); let y3=y+raggio*sin(a3); 
    ...
    window.keyPressed = function() {
        if (key >= '1' && key <= '5') {
            ...
            console.log("Stile Attivo:", currentStyle);
            updateControlsVisibility();
        }
    ...
    function updateControlsVisibility() {
        ...
        // Mostra solo i controlli per lo stile corrente
        switch (currentStyle) {
            case 1:
                select('#group-h').show();
